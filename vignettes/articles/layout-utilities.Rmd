...
- `wa_page()`, which creates a minimal page scaffold for Shiny apps using the
  package
...
`wa_page()` creates a minimal full-page HTML scaffold and attaches the
...
`wa_page()` is especially useful when you want a small, explicit page helper
...
ui <- wa_page(
  ...
  wa_container(
    class = "wa-stack",
    wa_card("Top card"),
    wa_button("primary_action", "Continue", appearance = "filled"),
    wa_card("Bottom card")
  )
)
...
ui <- wa_page(
  ...
  wa_container(
    ...
    wa_button("refresh", "Refresh", appearance = "outlined")
  )
)
...
These examples are good candidates for `shinylive` on the documentation
website because the value is primarily visual and structural.

# Notes

Use `wa_page()` when you want the package to own the full page scaffold.
