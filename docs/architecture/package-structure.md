...
Files generated during build that report on the build live in `build/` and `report/`. A current example is API coverage and conformance tracking (see details in `docs/development/component-coverage.md` and `docs/development/manifests.md`). Future additions may store other files in these directories.
...
* `dev`, `build` and `report` store files used for and generated by the build process
