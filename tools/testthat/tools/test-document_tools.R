...
.create_fake_repo <- function(root) {
  ...
  file.copy(
    normalizePath(
      file.path("..", "..", "check_interactive.R"),
      mustWork = TRUE
    ),
    file.path(root, "tools", "check_interactive.R")
  )
  file.copy(
    normalizePath(file.path("..", "..", "check_rhub.R"), mustWork = TRUE),
...
testthat::test_that(
  "document_tools default file set includes site and build-stage tools",
  {
    ...
    testthat::expect_true("update_version.Rd" %in% result$generated)
    testthat::expect_true(
    ...
    testthat::expect_true(
      file.exists(file.path(root, "tools", "man", "check_interactive.Rd"))
    )
    testthat::expect_true(
      file.exists(file.path(root, "tools", "man", "check_rhub.Rd"))
    )
    testthat::expect_true(
    ...
    testthat::expect_true(
      file.exists(file.path(root, "tools", "man", "update_version.Rd"))
    )
  }
)
